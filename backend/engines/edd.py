...
"""
edd.py
EDD (Earliest Due Date) 스케줄링 알고리즘

납기일 기준 정렬 후, 납기 내 완료 가능한 라인 중 여유가 가장 적은 라인 우선 배정
→ 납기 준수율 최대화에 집중
"""
...
from engines.mold_utils import (
    DEFAULT_MOLD_QUANTITIES, is_mold_feasible, register_mold,
...
def run_edd(orders, n_lines, n_slots, line_valid_models,
    ...
    # 납기 빠른 순 → 납기 같으면 수량 많은 순
    sorted_orders = sorted(orders, key=lambda o: (o["due"], -o["qty"]))
    ...
    for order in sorted_orders:
        ...
        # 납기 준수 가능한 라인: 배정 시작 + 필요 슬롯 <= due
        feasible = [l for l in valid_lines
                    if line_next_slot[l] + n_slots_needed <= due]

        # 가장 적게 배치된 라인 우선 (부하 최소 라인)
        if feasible:
            chosen_line = min(feasible, key=lambda l: line_next_slot[l])
        else:
